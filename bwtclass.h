...
public:
    ...
    void read_block(unsigned int position) {
        ...
        // Go through the buffer once and create buffer entries 
        unsigned int record_spacing = block_size / 10;
        unsigned int buffer_pos = 0;
        unsigned int curr_position = count_starting_point;
        while (buffer_pos < block_size) {
            auto current_entry = utility::readRlbEntry(*this, buffer_pos);
            printf("Character: %c, Count: %u\n", current_entry.character, current_entry.count);
        }
